...
function Init() {
        ...
        docker run -d --network host --restart unless-stopped -v /etc/localtime:/etc/localtime:ro --name $name -v /docker/redis/$name/conf:/etc/redis/  -v $name-data:/data zhugeqing/redis redis-server /etc/redis/redis.conf
        # 软链接
